...
func main(){
  numbersArray := [][]int{
    ...
    sum := 0
      for row := range numbersArray {
        remainderOfRow := divisible(numbersArray[row])
...
func divisible(numberList []int) int {
  for i := range numberList {
    for j := 0; j < len(numberList); j++ {
      if i != j && numberList[i] % numberList[j] == 0 {
